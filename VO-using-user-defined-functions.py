...
def calculateEssentialMatrix(calibrationMatrix, fundMatrix):
    tempMatrix = np.matmul(
        np.matmul(calibrationMatrix.T, fundMatrix), calibrationMatrix)
    u, s, v = np.linalg.svd(tempMatrix, full_matrices=True)
    # Constraining Eigenvalues to 1, 1, 0
...
def getTriangulationPoint(m1, m2, point1, point2):
    ...
    oldx = np.array([[0, -1, point1[1]], [1, 0, -point1[0]],
                    [-point1[1], point1[0], 0]])
    # Skew Symmetric Matrix of point2
    oldxdash = np.array(
        [[0, -1, point2[1]], [1, 0, -point2[0]], [-point2[1], point2[0], 0]])
...
def disambiguiousPose(Rlist, Clist, features1, features2):
    check = 0
    # current camera pose is always considered as an identity matrix
    ...
    for index in range(0, len(Rlist)):  # Looping over all the rotation matrices
        ...
        if angles[0] < 50 and angles[0] > -50 and angles[2] < 50 and angles[2] > -50:
            ...
            for i in range(0, len(features1)):  # Looping over all the inliers
                # Triangulating all the inliers
                temp1x = getTriangulationPoint(
                    Horigin[0:3, :], newP, features1[i], features2[i])
                thirdrow = Rlist[index][2, :].reshape((1, 3))
                # If the depth of the triangulated point is positive
                if np.squeeze(thirdrow @ (temp1x - Clist[index])) > 0:
                    count = count + 1
...
for index in range(19, len(images)-1):  # Looping over all the images
    ...
    # Read current image
    img1 = cv2.imread("stereo/centre/" + str(images[index]), 0)
    ...
    undistortedimage1 = UndistortImage(
        colorimage1, LUT)  # Undistort the current image
    # Convert the undistorted image in grayscale
    gray1 = cv2.cvtColor(undistortedimage1, cv2.COLOR_BGR2GRAY)

    # Read next image
    img2 = cv2.imread("stereo/centre/" + str(images[index + 1]), 0)
    # Convert image from bayer format to BGR format
    colorimage2 = cv2.cvtColor(img2, cv2.COLOR_BayerGR2BGR)
    undistortedimage2 = UndistortImage(
        colorimage2, LUT)  # Undistort the next image
    # Convert the undistorted image in grayscale
    gray2 = cv2.cvtColor(undistortedimage2, cv2.COLOR_BGR2GRAY)

    # Cropping the area of interest from the current frame
    gray1_crop_img = gray1[200:650, 0:1280]
    # Cropping the area of interest from the next frame
    gray2_crop_img = gray2[200:650, 0:1280]
    ...
    for i in range(0, 50):  # 50 iterations for RANSAC
        ...
        # Variable for storing corresponding eight random points from the next frame
        goodFeatures2 = []
        tempfeature1 = []
        tempfeature2 = []
    ...
    lastH = lastH @ Homogenousmatrix(R, T)
